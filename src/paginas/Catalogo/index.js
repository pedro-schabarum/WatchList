...
export default function Catalogo({ navigation }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const data = await fetchConteudos({
        endpoint,
        pagina,
        idioma,
      ...
      setConteudos((prevConteudos) => {
        const novosConteudos = data.results.filter(
          (filme) =>
            !prevConteudos.some((prevFilme) => prevFilme.id === filme.id)
        );
        return pagina === 1
          ? novosConteudos
          : [...prevConteudos, ...novosConteudos];
      });
    } catch (error) {
  ...
  return (
    <PaginaBase>
      ...
      <Lista conteudos={conteudos} handleLoadMore={handleLoadMore} />
